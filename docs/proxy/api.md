...
```python
from exectunnel.proxy import Socks5ServerConfig, Socks5Server, Socks5Request, UdpRelay
```
...
│  Enqueues one Socks5Request per completed handshake.  │
└───────────────────────────────────────────────────────┘
        │
        │  yields Socks5Request objects
        ▼
┌───────────────────────────────────────────────────────┐
│  Socks5Request                                        │
│  ─────────────────────────────────────────────────── │
...
The proxy layer hands you a `Socks5Request`. You open a tunnel connection
(session layer), send the reply, then relay bytes. The proxy layer never
touches the tunnel — that is entirely your responsibility.
...
> After `start()` returns the server is immediately accepting connections.
> Handshakes run concurrently in the background — you do not need to do
> anything to enable that.

---

#### `await server.stop() → None`

Closes the listen socket, cancels all in-flight handshakes, drains and closes
any unconsumed `Socks5Request` objects, then signals the `async for` iterator
to stop.
...
> Always call `stop()` (or use the async context manager) to avoid leaking
> the listen socket and any in-flight writer handles.
...
Yields `Socks5Request` objects one at a time as handshakes complete.
Stops when `stop()` is called.

**Critical:** spawn each request as a `Task` immediately. Do not `await`
anything inside the loop body — that blocks the accept loop and starves
all other clients.
...
You never construct `Socks5Request` directly. You receive it from
`async for req in server`.
...
Use these instead of `req.cmd == Cmd.CONNECT` — they are more readable and
insulate you from enum import boilerplate.
...
Always use `async with req` or call `await req.close()` explicitly.
Never rely on garbage collection to close the writer.

---

### Reply methods

You **must** call exactly one of `send_reply_success` or `send_reply_error`
per request. Calling neither leaks the connection. Calling both raises
`ProtocolError`.
...
Sends a SOCKS5 `SUCCESS` reply and flushes the writer. Does **not** close
the writer — you own the connection after this call.
...
```python
await req.send_reply_success(
    bind_port=req.udp_relay.local_port
)
```

Forgetting `bind_port` for UDP means the client sends datagrams to port 0
and the relay never receives them.
...
| Exception | When |
|---|---|
| `ProtocolError` | Writer already closing when called. |
...
Sends a SOCKS5 error reply, flushes the writer, then closes the writer and
any UDP relay. Use this when you cannot fulfil the request.
...
Drain and close errors are suppressed — the connection is being torn down
regardless.
...
|---|---|
| `ProtocolError` | Called a second time (double-reply guard). |
| `ConfigurationError` | `reply` is not a valid `Reply` member — this is a caller bug. |
...
```python
from exectunnel.proxy import Socks5Request
from exectunnel.exceptions import ExecTunnelError
from exectunnel.protocol import Reply

async def handle_connect(req: Socks5Request) -> None:
    async with req:
...
async def handle_udp(req: Socks5Request) -> None:
    async with req:
...
You never construct `UdpRelay` directly in normal usage. The server creates
and starts it for you; you receive it via `req.udp_relay`.

The only case where you construct it manually is in tests or if you are
building a custom server that bypasses `Socks5Server`.
...
In normal usage this is called by `Socks5Server._negotiate()` before the
`Socks5Request` is enqueued. You do not call this yourself unless you are
constructing `UdpRelay` manually.
...
Closes the UDP socket and unblocks any coroutine awaiting `recv()`.
Idempotent.
...
After `close()`, `recv()` drains any remaining queued items and then returns
`None` to signal EOF.
...
Blocks until a datagram is available or the relay is closed. Returns `None`
when the relay has been closed and the queue is fully drained — treat this
as EOF and stop the relay loop.
...
while True:
    ...
    flow_id = await session.open_udp_flow(host, port)
    await session.send_udp(flow_id, payload)
```

The `payload` is the raw datagram bytes with the SOCKS5 UDP header already
stripped. `host` and `port` are the **destination** the client wants to reach.
...
Call this when the remote side sends a UDP datagram back toward the client.
`src_host` and `src_port` are the **source** of the reply (the remote
service's address), not the client's address.
...
* `src_host` is not a valid IP address string — RFC 1928 §7 requires IP
  addresses in UDP reply headers; domain names are not permitted.
...
async def relay_udp(
    ...
    relay = req.udp_relay
    # Track open flows: dst_key → flow_id
    flows: dict[tuple[str, int], str] = {}
    ...
    async def pump_inbound() -> None:
        ...
        while True:
            ...
            key = (host, port)
            if key not in flows:
                flows[key] = await session.open_udp_flow(host, port)
            await session.send_udp(flows[key], payload)

    async def pump_outbound() -> None:
        """Tunnel → client: read from session, forward to relay."""
        async for flow_id, datagram, src_host, src_port in session.udp_datagrams():
    ...
    # Run all three concurrently; stop when any one finishes.
    tasks = {
    ...
    try:
        ...
        for flow_id in flows.values():
            await session.close_udp_flow(flow_id)
```
...
The proxy layer handles all SOCKS5 negotiation errors internally. By the time
...
* `BootstrapError` — no bootstrap happens in the proxy layer.
    ...
    async with req:
        try:
            ...
            logger.warning(
                ...
                exc.error_code, exc.message, exc.error_id,
            )
            if not req.replied:
...
> Use `req.replied` to guard `send_reply_error` in exception handlers —
> if `send_reply_success` already succeeded, calling `send_reply_error`
> raises `ProtocolError` (double-reply).
...
□  UDP flows closed in session layer when relay loop exits — proxy layer
   does not know about session-layer flow IDs.
...
├── await stop()
├── async with server
└── async for req in server → Socks5Request

Socks5Request                          (received from server, never constructed)
├── .cmd            Cmd
├── .host           str
├── .port           int
