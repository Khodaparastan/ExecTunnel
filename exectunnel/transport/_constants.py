...
:data:`exectunnel.protocol.MAX_FRAME_LEN` so there is exactly one source
...
Attributes:
    MAX_DATA_CHUNK_BYTES: Maximum raw byte count that can be carried in a
        ...
        ``(MAX_FRAME_LEN - FRAME_OVERHEAD_BYTES) * 3 // 4``.
...
from exectunnel.protocol import MAX_FRAME_LEN
